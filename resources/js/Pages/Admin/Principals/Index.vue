...
const submit = () => {
    if (isEditing.value) {
        form.put(route('admin.principals.update', editId.value), {
            onSuccess: () => reset()
        });
    } else {
        form.post(route('admin.principals.store'), {
            onSuccess: () => reset()
        });
    }
};
...
const destroy = (id) => {
    if (confirm('Are you sure you want to delete this principal?')) {
        router.delete(route('admin.principals.destroy', id));
    }
};
</script>
...
<template>
    ...
    <AdminLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Master Data: Principals</h2>
        </template>
